...
# Kullanıcıları saklamak için bir sözlük
user_data = {}
...
def process_input(client, message, text):
    chat_id = message.chat.id
    if user_data[chat_id]['state'] == 'phone':
        user_data[chat_id]['telno'] = text
        client.send_message(chat_id, "🔒Vodafone Şifreni Gir")
        user_data[chat_id]['state'] = 'password'
    elif user_data[chat_id]['state'] == 'password':
        parola = text
        telno = user_data[chat_id]['telno']
        user_data[chat_id]['state'] = 'otp'
        data = {
            "context": "e30=",
            "username": telno,
        ...
        user_data[chat_id]['proid'] = proid
        time.sleep(2)
        client.send_message(chat_id, "📨SMS ile Gelen Kodu Gir")
    elif user_data[chat_id]['state'] == 'otp':
        kod = text
        proid = user_data[chat_id]['proid']
...
def vf_cark(client, chat_id):
    ...
    cark_url = f"https://m.vodafone.com.tr/squat/getSquatMarketingProduct?sid={user_data[chat_id]['proid']}"
...
def vf_oyun(client, chat_id):
    ...
    o_url = f"https://m.vodafone.com.tr/marketplace?method=participateCampaignBE&sid={user_data[chat_id]['proid']}"
